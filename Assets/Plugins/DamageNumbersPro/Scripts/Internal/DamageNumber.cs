...
namespace DamageNumbersPro
{
    public abstract class DamageNumber : MonoBehaviour
    {
        ...
        public DamageNumber SpawnUIText(Transform parent, Vector2 anchorPostion, string text)
        {
            DamageNumber newDN = Spawn();
            newDN.SetAnchoredPosition(parent, anchorPostion);
            //Number:
            newDN.text = text;
            return newDN;
        }

        public DamageNumber SpawnText(Vector3 newPosition, string text)
        {
            DamageNumber newDN = Spawn();
