...
namespace Framework.Coroutine
{
    public class CoroutineHandler
    {
        ...
        private readonly Dictionary<Event, List<ActiveCoroutine>> eventCoroutines =
            new Dictionary<Event, List<ActiveCoroutine>>();

        private readonly HashSet<(Event, ActiveCoroutine)> eventCoroutinesToRemove =
            new HashSet<(Event, ActiveCoroutine)>();
        ...
        {
            return this.Start(CoroutineHandler.InvokeLaterImpl(wait, action), name, priority);
        }
